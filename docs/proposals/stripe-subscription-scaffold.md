> **Status: PROPOSAL — NOT IMPLEMENTED.** This was the repository README from
> 2026-09-17 (`cec782d`) to 2026-09-23. It describes a Next.js 14 + Prisma +
> Auth.js subscription app. Checked against the tree on 2026-09-23, **none of
> the 13 files it names exists** (`prisma/schema.prisma`, `lib/stripe.ts`,
> `app/api/checkout`, `app/api/webhook`, `__tests__/plans.test.ts`, ...), the
> root package is Next 15.5.25 rather than 14, and there is no
> `stripe:listen` script. The security controls it lists are design intent,
> not code. Kept verbatim because it may be a plan the owner means to build.
> Subscriptions that do exist today live in the FastAPI control plane; see
> `STRIPE_SETUP.md`.
