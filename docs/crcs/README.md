...
| S4 | **Stripe account status reconciled 2026-09-25:** `charges_enabled: true`, `payouts_enabled: true`, no currently-due/past-due requirements, and 0 registered webhook endpoints. Buyer-path completion is not verified and the live charge query returned 0 charges. | Read-only Stripe API reconciliation; `STRIPE_SETUP.md`, `STRIPE_LIVE_READINESS.md` |
