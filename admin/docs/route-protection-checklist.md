...
- Set `ADMIN_API_KEY` to the control plane's value. The admin server sends it as a bearer (`lib/api.ts`, server-only); without it every page reads the control plane's 401 as an empty list. In production, `/api/login` refuses all logins when neither `ADMIN_LOGIN_TOKEN` nor `ADMIN_API_KEY` is set.
- Every path needs a session unless it is listed in `PUBLIC_PATHS` in `lib/route-policy.ts` (default deny). Adding a public path is a security decision; `admin/tests/login-guard.test.mjs` pins the list's behaviour.
- Login `next` targets go through `safeNextPath` (`lib/login-guard.ts`); do not build a redirect from a raw query or form value.
...
- In production, verify unauthorized requests to `/`, `/approvals`, `/audit`, `/revenue`, `/playbooks`, `/premium`, `/api/premium/briefing`, and `/api/assets/sign` redirect or deny before any sensitive payload is returned.
